...
Los resultados de la Tabla \ref{table:CompVersionesFPS} reflejan la evolución positiva del algoritmo ya que a medida que se fue sofisticando el algoritmo se incrementaron los FPS para todos los casos de prueba.

El incremento del tiempo de generación de imagen es mayor en el primer cambio de versión, esta diferencia está determinada por el uso de la jerarquía de memoria de la GPU. En la Versión 1 todos los accesos a memoria son a memoria global mientras que en la Versión 2 la mayoría de los datos de entrada del algoritmo de Raytracing se encuentran en memoria de textura. Considerando estos casos de prueba, el correcto uso de la jerarquía de memoria permite que el algoritmo pierda menos tiempo accediendo a memoria siendo así (en promedio) tres veces y media más rápido que el algoritmo que no la usa.
...
En las Tablas \ref{table:EquiposEquipo1}, \ref{table:EquiposEquipo2} y \ref{table:EquiposEquipo3} se muestran los resultados obtenidos para los equipos 9500M, 9600M y GTX260 respectivamente.
...
/////////////////////RE REDACTAR COMPARACIÓN CON GHÜNTER ET AL.///////////////////////////////////
Un caso de prueba que permite evaluar un resultado positivo en la implementación del algoritmo de Raytracing en CUDA es la escena BUNNY. En el trabajo de Günther et al. \cite{GuntherPopov} se hizo una implementación similar a la de este proyecto del algoritmo de Raytracing. En este trabajo se generan renders de una escena que es igual a la escena BUNNY y se presentan resultados de tiempos de generación de imágenes. Debido a que solo se presentan resultados y no se tiene acceso al código fuente, las pruebas con el algoritmo implementado se adaptaron al trabajo de Günther et al. para lograr resultados comparables.
