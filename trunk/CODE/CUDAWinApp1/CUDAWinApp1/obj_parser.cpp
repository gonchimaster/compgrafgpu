...
int obj_parse_mtl_file(char *filename, list *material_list)
{
	...
	while( fgets(current_line, OBJ_LINE_SIZE, mtl_file_stream) )
	{
		...
		//refraction
		else if( strequal(current_token, "refract") && material_open)
		{
			current_mtl->refract = atof( strtok(NULL, " \t"));
		}
		//glossy
...
/*		
	...
	while( fgets(current_line, OBJ_LINE_SIZE, obj_file_stream) )
	{
		...
		else if( strequal(current_token, "mtllib") ) // mtllib
		{
			...
			//Agregado...
			char c = '\\';
			char* resto = strrchr(filename, c);
			if(resto != 0){
				int aSacar = (int)strlen(resto);
				size_t aCopiar = strlen(filename) - aSacar;
				char* pathCompleto = (char*) malloc(sizeof(char) * (aCopiar + 2 + strlen(growable_data->material_filename)));
				strncpy(pathCompleto, filename, aCopiar);
				pathCompleto[aCopiar] = '\\';
				pathCompleto[aCopiar + 1] = '\0';
				strcat(pathCompleto, growable_data->material_filename);
				obj_parse_mtl_file(pathCompleto, &growable_data->material_list);
				free(pathCompleto);
			}
			else{
			//FIN
				obj_parse_mtl_file(growable_data->material_filename, &growable_data->material_list); //para volver al original dejar solo esta linea.
			}
			continue;
		}
